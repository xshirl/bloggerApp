...
function register(credentials) {
  ...
  return bcrypt.hash(credentials.password, saltRounds).then((hash) => {
    const newUser = {
...
function login(credentials) {
  ...
  return findByEmail(credentials.email).then((user) =>
    // bcrypt using magic compares provided password with password digest
    bcrypt.compare(credentials.password, user.pw_digest).then((match) => {
